...
(function () {
    ...
    for (const i in currentAnimals) {
        ...
        newEm.setAttribute('data-category', ((+i + 1) % 2 === 0) ? 'seagreen' : 'salmon');
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const foldingContainer = new Portfolio('.content');
    });
})();
...
class Portfolio {
    ...
    addFilteringButtonsListener() {
        document.querySelectorAll(`${this.options.filters.selector} *[data-filter]`).forEach(filter => {
    ...
    addSearchFieldListener() {
        const searchField = document.querySelector(`${this.options.filters.selector} *[data-search]`);
    ...
    initFiltering(category = 'all', searchText = null) {
        ...
        this.isContainerVisible = window.innerHeight - this.em.getBoundingClientRect().y > 0;

        // Deselect the currently selected filters (remove CSS class)
        document.querySelectorAll(`${this.options.filters.selector} *[data-filter].${this.options.filters.activeClass}`).forEach(activeFilter => activeFilter.classList.remove(this.options.filters.activeClass));
        // Select the clicked filter (add CSS class)
        document.querySelector(`${this.options.filters.selector} *[data-filter~="${this.currentFilter.category}"]`).classList.add(this.options.filters.activeClass);
    ...
    filterContainerItems() {
        // Show all the items
        this.em.querySelectorAll(this.options.itemSelector).forEach(item => item.style.display = '');

        // If a specific filter was chosen, hide the ones that don't contain it.
        if (this.currentFilter.category !== 'all') {
            this.em.querySelectorAll(`${this.options.itemSelector}:not([data-category~="${this.currentFilter.category}"])`).forEach(item => {
        ...
        if (this.currentFilter.searchText) {
            ...
            const selector = this.options.search.textSelector || this.options.itemSelector;

            Array.from(this.em.querySelectorAll(selector))
                .filter(item => {
                    if (this.options.search.isCaseSensitive) {
                        return !item.innerText.includes(this.currentFilter.searchText);
                    }
                    else {
                        return !item.innerText.toLowerCase().includes(this.currentFilter.searchText.toLowerCase());
                    }
                })
                .forEach(item => {
                    item.closest(this.options.itemSelector).style.display = 'none';
                });
        }
    ...
    fold() {
        ...
        const currentMaxHeight = window.innerHeight - this.em.getBoundingClientRect().y;
        ...
        this.animation = this.em.animate({
            height: [`${currentHeight < currentMaxHeight ? currentHeight : currentMaxHeight}px`, 0],
        }, {
    ...
    unfold() {
        ...
        const baseHeight = gridRows.reduce((prev, curr) => prev = +prev + +curr); // Add the height of all the rows
        const nextHeight = (+styles.gridColumnGap.replace('px', '') * (gridRows.length - 1)) + baseHeight; // Add the gaps size to the previouly calculated rows height
        const maxVisibleHeight = window.innerHeight - this.em.getBoundingClientRect().y; // Calculate the visible height of the container
        ...
        // Animation (see https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API)
        this.animation = this.em.animate({
            height: [0, `${(nextHeight < maxVisibleHeight) ? nextHeight : maxVisibleHeight}px`],
        }, {
                duration: this.options.animationDuration / 2,
                easing: 'ease-out'
            });
